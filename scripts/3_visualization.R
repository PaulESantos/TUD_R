...
pkgs <- c("ggplot2", "readr", "tibble", "dplyr", "forcats", "stringr", "scales", 
          "systemfonts", "scico", "rcartocolor", "RColorBrewer", "prismatic",
          "ggtext", "patchwork", "ggrepel", "ggforce", "sf", "devtools")
unavailable <- setdiff(pkgs, rownames(installed.packages()))
install.packages(unavailable)

## Note: The download functionality of {rnaturalearth} currently only works
##       with the development version, available via GitHub:
devtools::install_github("ropensci/rnaturalearth")

## We are using non-default font in this session. If you want to run all code
## "as it is", please install the following fonts which are available for free:
##  - Asap Condensed 
##  - Cabinet Grotesk
##  - Pally
##
...
## -----------------------------------------------------------------------------
ggplot(bikes, aes(x = year, y = count)) +
  geom_point(
    aes(color = season),
    alpha = .3
  )
...
ggplot(bikes, aes(x = year, y = count)) +
  geom_col(
    ...
    position = position_dodge(
      width = .8  ## by default same as col width
    )
  )
...
system_fonts() %>%
  filter(stringr::str_detect(family, "Cabinet")) %>%
  pull(name) %>%
  sort()
...
## -----------------------------------------------------------------------------
ggplot(bikes, aes(temp_feel, y = count)) +
  geom_point(
    aes(color = day_night)
  ) +
  stat_smooth(
...
# install.packages("prismatic")
library(prismatic)
clr_lighten(colors, .5)


## -----------------------------------------------------------------------------
clr_desaturate(colors, .3)


## -----------------------------------------------------------------------------
clr_desaturate(clr_lighten(colors, .5), .3)
...
ggplot(bikes, aes(temp_feel, y = count)) +
  geom_point(
    aes(
      color = day_night,
      color = after_scale(clr_lighten(color, .5))
    )
  ) +
  stat_smooth(
    aes(color = day_night),
...
ggplot(bikes, aes(temp_feel, y = count)) +
  geom_point(
    aes(
      color = day_night,
      color = after_scale(
        clr_desaturate(clr_lighten(color, .5), .3)
      )
    )
  ) +
  stat_smooth(
...
ggplot(bikes, aes(temp_feel, y = count)) +
  geom_point(
    aes(
      color = stage(
        day_night,
        after_scale = clr_desaturate(clr_lighten(color, .5), .3)
      )
    )
  ) +
  stat_smooth(
...
## -----------------------------------------------------------------------------
theme_std <- theme_set(theme_minimal(base_size = 20, base_family = "Pally"))
theme_update(
  text = element_text(family = "Pally"),
  panel.grid = element_blank(),
  axis.text = element_text(color = "grey50", size = rel(.65)),
  axis.title = element_text(color = "grey40", face = "bold"),
  axis.title.x = element_text(margin = margin(t = 12)),
  axis.title.y = element_text(margin = margin(r = 12)),
  axis.line = element_line(color = "grey80", size = .4),
  legend.text = element_text(color = "grey50", size = rel(.65)),
  plot.title = element_text(face = "bold"),
  plot.tag = element_text(size = rel(1.5), margin = margin(b = 15))
)
...
p1 <- 
  ...
  labs(
    x = NULL, y = "Sum of reported bike shares"
  ) +
  theme(
...
## -----------------------------------------------------------------------------
((p1 + p2) / p3 & theme(legend.position = "none")) +
  plot_layout(heights = c(1.5, 1), widths = c(2, 1))


## -----------------------------------------------------------------------------
((p1 + p2) / plot_spacer() / p3 & theme(legend.position = "none")) +
  plot_layout(heights = c(1.5, .1, 1), widths = c(2, 1))
...
## -----------------------------------------------------------------------------
pt1 <- p1 + labs(title = "Plot One") + theme(legend.position = "none")
pt2 <- p2 + labs(title = "Plot Two") + theme(legend.position = "none")
pt3 <- p3 + labs(title = "Plot Three") + theme(legend.position = "none")
...
## -----------------------------------------------------------------------------
pt1 + inset_element(pt2, l = .5, b = .1, r = 1, t = .6)


## -----------------------------------------------------------------------------
pt1 + inset_element(pt2, l = .6, b = 0, r = 1, t = .5, align_to = 'full')


## -----------------------------------------------------------------------------
(pt1 + inset_element(pt2, l = .5, b = .05, r = 1, t = .65)) / pt3 + 
...
p <- 
  ...
  theme_minimal(base_size = 18, base_family = "Asap Condensed") +
  theme(
    legend.position = "none",
    panel.grid.major.x = element_blank(),
    panel.grid.minor = element_blank(),
...
p +
  geom_point(
    ...
    position = position_jitterdodge(
      jitter.width = .2, dodge.width = .6, seed = 1
    ),
    alpha = .5
  ) +
  stat_summary(
  ...
  stat_summary(
    ...
    position = position_dodge(width = .6),
    hjust = -.2, size = 5.5, family = "Asap Condensed", fontface = "bold"
  )
...
p +
  ggforce::geom_sina(
    aes(color = stage(
      day_night,
      after_scale = clr_lighten(color, .6)
    )),
    position = position_dodge(width = .6),
    alpha = .5
  )  +
